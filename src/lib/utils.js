 ...
 * @returns {string}
 */
export function maybeEscape(unsafe) {
    if (unsafe == undefined) {
        return unsafe;
    }
...
export function safeFilename(str, extension = '') {
	if (extension) {
...
export function filenameForUrl(url, extension = '') {
...
export function normalizeUrl(url, defaultOrigin) {
    return new URL(normalizeRelativePath(url), defaultOrigin).toString()
}

export function normalizeRelativePath(url) {
 ...
 */
export function getUrlObj(url, baseUrl) {
 ...
 * @returns 
 */
export function maybeMirrorUrl(u, defaultOrigin = undefined, localArchiveMirror) {
